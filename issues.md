...
The table under **Current benchmark backlog** is the authoritative live status.
The 43-item register later in this file is preserved as the historical audit of
the inherited baseline; its original "MVP status" values are not current
implementation evidence.

Status vocabulary:

- `open`: the required behavior is absent or materially incorrect;
- `partial`: a useful component exists, but the acceptance gate is not met;
- `implemented`: code and focused tests exist;
- `mock-observed`: implementation has also been exercised in the deterministic
  rehearsal;
- `deferred`: intentionally outside the current authorized scope.

## Current benchmark backlog
...
| BEN-006 | Critical | M–L | 3 | mock-observed | Provider attempts have explicit reserved, started, sent, acknowledged, reconciled, rejected, definitely-not-sent, and billing-unknown states. Sent timeouts, disconnects, malformed responses, and identity drift retain worst-case batch capacity. |
| BEN-007 | High | M | 3 | mock-observed | Completion tokens include reasoning as a measured subset; totals and estimates do not add reasoning twice. Missing provider cost remains null and receives a nonzero price-snapshot estimate when applicable. |
...
| BEN-011 | High | L | 1 | mock-observed | The current schema uses condition-aware trajectory fields for graph/trap entry, trap-directed actions, completed cycles, exit, task resumption/completion, and re-entry. Recognition remains secondary. |
| BEN-012 | High | L | 3 | mock-observed | Authoritative end-of-trial operator, scaffold, and model ledgers cover attempts, billing state, known/maximum cost, token categories, actions, requests/redirects, generated/sent/delivered bytes, nodes, depth, and wall time. Every field has a pair delta; unavailable post-calibration fields remain null. |
| BEN-013 | High | L | 4 | mock-observed | Artifacts retain recursively redacted exact request/response envelopes with raw digests, write atomically, bind scorer/software/fixture/apparatus digests, compare stored and replayed results, enforce stream/lifecycle invariants, and emit a sanitized pair evidence digest outside each raw trial directory. Release signing and public immutable storage remain later release work. |
| BEN-014 | Medium–high | M | 4 | mock-observed | Lifecycle transitions use expected-prior-state compare-and-swap, constructor failures are retained with sanitized diagnostics, cancellation finalizes then re-raises, and export failure stores an explicit artifact-unavailable outcome. Catastrophic database loss remains necessarily unrecoverable in that database. |
...
## Historical baseline issue register
